...
public class TrustManagemer : MonoBehaviour
{
    ...
    public float trustUpdate(int nbDeathsCurrentTurn) {
        ...
        float deathScore = float.MaxValue((expectedDeaths - nbDeathsCurrentTurn) / expectedDeaths, -1);
        ...
        trustLevel = float.MaxValue(float.MinValue((1 + deathScore/2) * trustLevel, 100), 0);
